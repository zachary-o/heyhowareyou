...
export default function TopOpener({
  opener,
  currentUserId,
}: {
  opener: OpenerType;
  ...
  const isCurrentUserAuthor = currentUserId && currentUserId === opener.user_id;

  return (
    <div
      className={`rounded-2xl border ${isCurrentUserAuthor ? "border-violet-500/20" : "border-white/10"} bg-white/4 backdrop-blur-xl p-5 transition-all duration-200 hover:border-white/20 hover:bg-white/6`}
    >
      <p
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-1.5">
          {isCurrentUserAuthor && (
            <span className="text-xs text-violet-500 px-2 py-1 rounded-lg border border-violet-600">
              yours
            </span>
          )}
